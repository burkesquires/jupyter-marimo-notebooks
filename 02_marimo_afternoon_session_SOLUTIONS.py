...
@app.cell
def _(chol_threshold, df_filtered, plt):
    ...
    scatter_ax.axhline(
        chol_threshold.value,
        color="red",
        linestyle="--",
        label=f"Threshold ({chol_threshold.value})",
    )
...
@app.cell
def _(mo):
    mo.md("✅ **Solution to Exercise 4.1** — a third control, feeding the plot above:")
    return


@app.cell
def _(mo):
    chol_threshold = mo.ui.slider(
        100, 350, value=240, step=10, label="Cholesterol threshold (mg/dL)"
    )
    chol_threshold
    return (chol_threshold,)
...
def _(mo):
    mo.md(
        r"""
        Note the file order here: this cell defining `chol_threshold` appears
        *after* the scatter-plot cell that uses it. That's fine — marimo
        executes by dependency graph, not file position (Module 2). Moving
        the slider above re-runs the scatter-plot cell even though it's
        "earlier" in the file.
        """
    )
    return
...
@app.cell
def _(mo):
    mo.md("✅ **Solution to Exercise 5.1** — grouped by `sex` instead of `site`:")
    return
